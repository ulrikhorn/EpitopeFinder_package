...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, error = FALSE, tidy.opts=list(width.cutoff=60), tidy=TRUE)
```
...
Epitope_finder <- function(Hydrophobicity_predictor_output, rm_hydrophobic_regions = FALSE){
  ...
  epitope_finder_df <- 
    ...
    data_frame("Description" = epi_seqs, 
               "Epitope_structural_location" = is_hydro_vector, 
               "Start (residue)" = start_end[seq(from = 1, to = length(start_end), by = 2)], 
               "End (residue)" = start_end[seq(from = 2, to = length(start_end), by = 2)])
